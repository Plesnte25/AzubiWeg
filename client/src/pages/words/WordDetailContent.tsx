...
/**
 * Word Detail's real content — split out of the routed WordDetail.tsx so
 * the lg master-detail layout (Vocabulary.tsx, German Companion
 * Desktop.dc.html id="1b") can embed the exact same real behavior inline
 * next to the word list, instead of navigating away from it. `embedded`
 * drops the back-button row and the full-viewport chrome (the detail pane
 * already sits inside its own bounded column) — everything else (drill,
 * word family, notes, flag/delete) is identical either way.
 */
export function WordDetailContent({ id, embedded = false }: { id: string; embedded?: boolean }) {
