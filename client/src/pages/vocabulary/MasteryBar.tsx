...
export function MasteryBar({ allWords, reviewsToday, totalReviews, onOpenAnalytics }: MasteryBarProps) {
  ...
  return (
    <div className="sticky top-0 z-[14] -mx-1 rounded-xl bg-card/95 px-4 py-3 shadow-md backdrop-blur">
      <div className="flex flex-wrap items-center gap-3">
        <span className="text-body font-semibold">{masteredPercent}%</span>
        <span className="text-micro font-bold tracking-wide text-ink-400">GEMEISTERT</span>

        <div className="flex h-[22px] flex-1 min-w-32 overflow-hidden rounded-full bg-paper text-micro font-medium text-white">
          {newCount > 0 && (
            <div
        ...
        <button
          className="flex shrink-0 items-center gap-1.5 rounded-md border border-hairline px-2.5 py-1.5 text-caption font-medium text-ink-600 hover:border-brand-400 hover:text-ink-900"
